...
def get_roi_td_bu_blob_names(is_training=True):
    """super cls blob names."""
    # rois blob: holds R regions of interest, each is a 5-tuple
    # (batch_idx, x1, y1, x2, y2) specifying an image batch index and a
    # rectangle (x1, y1, x2, y2)
    blob_names = ['roi_td_bu']
    if is_training:
        # labels_int32 blob: R categorical labels in [0, ..., K] for K
        # foreground classes plus background
        blob_names += ['labels_int32_roi_td_bu']
    ...
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
        ...
        for lvl in range(k_min, k_max + 1):
            blob_names += ['roi_td_bu_fpn' + str(lvl)]
        blob_names += ['roi_td_bu_idx_restore_int32']
        if is_training:
...
def add_roi_td_bu_blobs(blobs, rois, label=None):
...
def _sample_rois(rois, label):
    ...
    labels_int32_roi_deep_sup='labels_int32_roi_td_bu'
    roi_deep_sup='roi_td_bu'
...
def _add_multilevel_rois(blobs):
    ...
    _distribute_rois_over_fpn_levels('roi_td_bu')
    if cfg.MODEL.MASK_ON and cfg.MRCNN.AT_STAGE == 1:
