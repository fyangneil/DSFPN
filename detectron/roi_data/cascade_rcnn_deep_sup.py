...
def get_roi_cascade_deep_sup_blob_names(stage,is_training=True):
    ...
    blob_names = ['roi'+stage_name+'_deep_sup']
    if is_training:
    ...
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
        ...
        for lvl in range(k_min, k_max + 1):
            blob_names += ['roi'+stage_name+'_deep_sup_fpn'+str(lvl)]
        blob_names += ['roi'+stage_name+'_deep_sup_idx_restore_int32']
        if is_training:
...
def add_roi_cascade_deep_sup_blobs(blobs, rois,stage, label=None):
    ...
    for i in range(im_ids.size):
        ...
        frcn_blobs = _sample_rois(rois[rois_ind,:],label[rois_ind],stage)
...
def _sample_rois(rois, label,stage):
    ...
    labels_int32_roi_deep_sup='labels_int32_roi'+stage_name+'_deep_sup'
    roi_deep_sup='roi'+stage_name+'_deep_sup'
...
def _add_multilevel_rois(blobs,stage):
    ...
    _distribute_rois_over_fpn_levels('roi'+stage_name+'_deep_sup')
    if cfg.MODEL.MASK_ON and cfg.MRCNN.AT_STAGE == 1:
