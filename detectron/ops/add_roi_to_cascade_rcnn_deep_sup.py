...
# import pydevd
class AddRoiCascadeDeepSupOp(object):
    ...
    def forward(self, inputs, outputs):
        ...
        if self._train:
            rois = inputs[0].data
            label=inputs[1].data
            output_blob_names = roi_cascade_deep_sup_data.get_roi_cascade_deep_sup_blob_names(self._stage)
            blobs = {k: [] for k in output_blob_names}
            roi_cascade_deep_sup_data.add_roi_cascade_deep_sup_blobs(blobs, rois,self._stage,label)
