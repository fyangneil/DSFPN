...
def add_mask_rcnn_deep_sup_outputs(model, blob_in, dim):
    ...
    if cfg.MRCNN.USE_FC_OUTPUT:
        ...
        if cfg.MRCNN.UPSAMPLE_RATIO > 1:
            blob_out = model.BilinearInterpolation(
                'mask_deep_sup_fcn_logits', 'mask_deep_sup_fcn_logits_up', num_cls, num_cls,
                cfg.MRCNN.UPSAMPLE_RATIO
            )

    if not model.train:  # == if test
        blob_out = model.net.Sigmoid(blob_out, 'mask_deep_sup_fcn_probs')

    return blob_out


def add_mask_rcnn_deep_sup_losses(model, blob_mask):
    """Add Mask R-CNN specific losses."""
    loss_mask = model.net.SigmoidCrossEntropyLoss(
        [blob_mask, 'masks_deep_sup_int32'],
        'loss_mask_deep_sup',
        scale=model.GetLossScale() * cfg.MRCNN.WEIGHT_LOSS_MASK
    )
    loss_gradients = blob_utils.get_loss_gradients(model, [loss_mask])
    model.AddLosses('loss_mask_deep_sup')
...
def mask_rcnn_fcn_head_v1upXconvs(
    ...
    current = model.RoIFeatureTransform(
        blob_in,
        blob_out='_[mask]_roi_deep_sup_feat',
    ...
    blob_mask = model.Relu('conv5_mask_deep_sup', 'conv5_mask_deep_sup')
...
def mask_rcnn_fcn_head_v1upXconvs_gn(
    ...
    current = model.RoIFeatureTransform(
        blob_in,
        blob_out='_mask_roi_deep_sup_feat',
        blob_rois='mask_rois_deep_sup',
        method=cfg.MRCNN.ROI_XFORM_METHOD,
        resolution=cfg.MRCNN.ROI_XFORM_RESOLUTION,
        sampling_ratio=cfg.MRCNN.ROI_XFORM_SAMPLING_RATIO,
...
def mask_rcnn_fcn_head_v0upshare(model, blob_in, dim_in, spatial_scale):
    ...
    if model.train:  # share computation with bbox head at training time
        ...
        blob_conv5 = model.net.SampleAs(
            ['res5_2_sum', 'roi_has_mask_deep_sup_int32'],
